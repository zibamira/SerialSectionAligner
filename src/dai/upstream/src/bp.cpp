...
Real BP::run() {
    ...
    for( ; _iters < props.maxiter && maxDiff > props.tol && (toc() - tic) < props.maxtime; _iters++ ) {
        ...
        maxDiff= maxDiffBetweenMessages();
...
double BP::maxDiffBetweenMessages()
{
    int numVars = fg().nrVars();
    std::map<int,std::vector<Factor> > maxMarginals;
    std::map<int,double > maxDiffs;

    double absMaxDiff = 0.0;
   
    const std::vector<Factor>& allFactors=factors();
    for (int i=0;i<allFactors.size();i++)
    {
    
        Factor curFactor = beliefF( i);
        ...
        for (int j=0;j<curVars.size(); j++)
        {
            ...
            std::map<int,std::vector<Factor> >::iterator oldMaxMarginal= maxMarginals.find(curVars[j].label());
            std::map<int,std::vector<Factor> >::iterator end = maxMarginals.end();
            if(oldMaxMarginal == end)
            {
                std::vector<Factor> newVector;
                newVector.push_back(curMaxMarginal);
                maxMarginals.insert(pair<int,std::vector<Factor> >(curVars[j].label(), newVector));
            }
            else
            {
                ...
                for(int k=0;k<oldMarginals.size();k++)
                {
                    ...
                    double distance = dist(oldMarginals[k].normalized(), curMaxMarginal.normalized(),DISTLINF);
                    if(distance>absMaxDiff)
                        absMaxDiff= distance;
