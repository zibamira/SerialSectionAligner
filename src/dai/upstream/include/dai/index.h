    ...
    public:
        ...
        State( const VarSet &vs, BigInt linearState=0 ) : state(linearState), states() {
            if( linearState == 0 )
                ...
                for( VarSet::const_iterator v = vs.begin(); v != vs.end(); v++ ) {
                    states[*v] = BigInt_size_t( linearState % (unsigned long)v->states() );
                    linearState /= (unsigned long)v->states();
                }
        ...
        BigInt operator() ( const VarSet &vs ) const {
            ...
            for( VarSet::const_iterator v = vs.begin(); v != vs.end(); v++ ) {
                states_type::const_iterator entry = states.find( *v );
                if( entry != states.end() )
                    vs_state += (unsigned long)entry->second * prod;
                prod *= (unsigned long)v->states();
            }
            return vs_state;
        }
