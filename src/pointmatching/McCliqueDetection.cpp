...
void 
McCliqueDetection::computeCliquesBasic(
            McDArray<int>        & clique, 
            ...
            int                  & nCliques)
{
    ...
    while ( cand.size()>0 ) {
        ...
        computeCliquesBasic(clique, notList2, cand2, connected, nCliques);
...
void 
McCliqueDetection::computeCliquesBasicImproved(
            McDArray<int>        & clique, 
            ...
            int                  & nCliques)
{
    ...
    while ( endNot != endCand ) {
        ...
        computeCliquesBasicImproved(clique, notAndCand2, endNot2, endCand2, 
                         connected, nCliques);
...
bool
McCliqueDetection::computeCliquesBronKerbosch(
            McDArray<int>            & clique, 
            McDArray<int>            & notAndCand, 
            int                        endNot, 
            int                        endCand,
            ...
            int                        maxNumCliques)
{
    if ( maxNumCliques > -1 && nCliques >= maxNumCliques ) {
        return false;
    }
    
    McDArray<int> notAndCand2;
    
    int i, j, endNot2, endCand2, count, pos=-1, fixp, p, s, sel;
    int minNod = endCand;
    int nod = 0;
    ...
    for ( nod=minNod+nod; nod>0; nod-- ) {
        ...
        if ( endCand2 == 0 ) {
            ...
            if ( clique.size()+(endCand2-endNot2) >= minCliqueSize ) {
                if ( !computeCliquesBronKerbosch(clique, notAndCand2, endNot2, endCand2,
